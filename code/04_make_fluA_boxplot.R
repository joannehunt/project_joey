library(ggplot2)
library(dplyr)
here::i_am("code/04_make_fluA_boxplot.R")
data <- readRDS(file = here::here("output/variablized_cleaned_data.rds"))

fluA_ct_symptom_group_filtered <- data %>%
  filter(!is.na(flua_ct1), !is.na(symptom_duration_groups), !fluA_result_factor=="Negative")

boxplot_fluA <- ggplot(fluA_ct_symptom_group_filtered, aes(x = symptom_duration_groups, y = flua_ct1, fill = symptom_duration_groups)) +
  ...
  labs(
    title = "Comparison of Flu A Ct Values Across Symptom Duration Groups",
